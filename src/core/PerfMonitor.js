...
const PerfMonitor = (() => {
    const FRAME_BUDGET_MS = 16.6;          // 60 FPS target
    const WARN_THRESHOLD_MS = 15.0;         // avisa se subsistema > 5ms
    const HISTORY_SIZE = 120;              // últimos 2 segundos de frames
    ...
    function frameEnd() {
        ...
        if (dt > FRAME_BUDGET_MS) {
            _slowFrameCount++;
            // Removemos o aviso automático a cada frame lento para não floodar o celular
            // Mas os dados continuam sendo salvos para o relatório!
        }
    ...
    function markEnd(name) {
        ...
        // Aviso no console se um sistema sozinho pesar demais
...
// Expõe globalmente para você chamar no console do celular
